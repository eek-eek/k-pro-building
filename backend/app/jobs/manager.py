...
class JobManager:
    ...
    def _execute(self, runtime: JobRuntime, inp: BuildingInput) -> None:
        db = SessionLocal()
        try:
            # Запись running-строки в БД — уже в рабочем потоке, не на loop.
    ...
    def _persist_error(self, runtime: JobRuntime) -> None:
        ...
        try:
            with SessionLocal() as edb:
                ...
                if row is None:
                    row = Job(id=runtime.id, estimate_id=runtime.estimate_id)
                    edb.add(row)
                row.status = "error"
    ...
    async def events(self, job_id: str) -> AsyncIterator[JobStatus]:
        ...
        try:
            # начальный снапшот (текущее состояние) — даже для позднего клиента
