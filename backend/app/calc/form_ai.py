...
ФИЗИКА (обязательно). Здание подчиняется гравитации и опирается на грунт:
- нижний ярус всегда стоит на земле — хотя бы один блок имеет base=0; здание НЕ может
  парить, левитировать или висеть в воздухе без опор;
- каждый приподнятый блок (base>0) должен опираться на нижележащий объём (стилобат/ядро),
  а не висеть отдельно; вылеты-консоли — в разумных пределах;
- невозможны: парящие/летающие здания, опора на одну точку, перевёрнутая пирамида на
  острие, бесконечная высота, отрицательные/нулевые размеры.
Фантазийные/физически невозможные пожелания (напр. «здание парит над землёй»,
«дом-робот, висящий в воздухе») сведи к ближайшему РЕАЛИЗУЕМОМУ варианту, стоящему на
земле (напр. силуэт робота из блоков на фундаменте): status="adjusted", и в message
честно скажи, ЧТО невозможно (парение) и что сделано вместо этого. Если идею вообще
нельзя свести к зданию — status="rejected".
