...
def _seismic_floor_cap(balls: int) -> int | None:
    ...
    Жёсткие нормы Строительного кодекса РК №253-VIII: на селе-/оползнеопасных участках —
    не выше 3 этажей, в г. Алматы на таких участках — запрет новой застройки (ст. 78);
    выбор площадки по картам сейсмомикрозонирования (ст. 34). Эти зоны точечно не
    моделируем (нет слоя селе/оползневых карт) — здесь только индикатив по баллам."""
    if balls >= 10:
        return 5
    if balls == 9:
        return 9
    ...
    if dist <= AVOID_M:
        return FaultVerdict(
            ...
            note=(f"Точка в зоне разлома «{name}» (~{dist_i} м). Капитальное строительство "
                  ...
                  f"спец. ИГИ и сейсмомикрозонирование (ст. 34 Строительного кодекса РК — "
                  f"выбор благоприятной площадки). При вынужденной застройке — только "
    ...
    if dist <= CAUTION_M:
        ...
        return FaultVerdict(
            ...
            note=(f"Близко к разлому «{name}» (~{dist_i} м), сейсмичность {balls} баллов. "
                  ...
                  f"предусмотреть усиленную сейсмозащиту (СП РК 2.03-30; ст. 34 и 78 "
                  f"Строительного кодекса РК)."))
